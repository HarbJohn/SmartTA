# app1.py — SmartTA RAG (AUB Edition + SmartTA_Extensions wired-in correctly)
...
# expose "backend", "frontend", "utils" like in your friend's app
if str(SMARTTA_EXTENSIONS) not in sys.path:
    sys.path.insert(0, str(SMARTTA_EXTENSIONS))

# paths for your chat logs (RAG)
...
def find_aub_logo() -> str | None:
    p = SMARTTA_RAG_ROOT / "UI" / "aub_logo.png"
    return str(p) if p.exists() else None

def _tuple_key_dict_to_str_key(d: dict) -> dict:
    out = {}
    for k, v in (d or {}).items():
        if isinstance(k, tuple): out[f"{k[0]}:{k[1]}"] = v
        else: out[str(k)] = v
    return out
...
def append_log(entry: dict):
    ...
    if entry.get("img_path"): entry["img_path"] = True
...
def read_logs_df() -> pd.DataFrame:
    ...
    if "qid" not in df.columns:
        ...
        mask = df["qid"].isna(); n = int(mask.sum())
        if n: df.loc[mask, "qid"] = [str(uuid.uuid4()) for _ in range(n)]
    ...
    return df.sort_values("ts").drop_duplicates(subset=["qid"], keep="last")
...
# small context manager to temporarily chdir into SmartTA_Extensions (so "data/..." resolves correctly)
...
if mode == "Student Chat":
    ...
    if export_pdf and st.session_state.chat_history and REPORTLAB_OK:
        ...
        try:
            ...
            ParagraphStyle = None; TA_CENTER = 1; HRFlowable = None; colors = None
        title_style = ParagraphStyle('TitleCenter', parent=styles.get('Title', styles['Normal']),
                                     alignment=TA_CENTER, fontSize=26, leading=30, spaceAfter=12)
        subtitle_style = ParagraphStyle('SubCenter', parent=styles.get('BodyText', styles['Normal']),
                                        alignment=TA_CENTER, fontSize=14, leading=18, spaceAfter=8)
        meta_style = ParagraphStyle('MetaCenter', parent=styles.get('Italic', styles['Normal']),
                                    alignment=TA_CENTER, fontSize=10, leading=12, spaceAfter=12)
        qa_q_style = ParagraphStyle('QAQ', parent=styles.get('BodyText', styles['Normal']),
                                    fontSize=11, leading=14, spaceAfter=4)
        qa_a_style = ParagraphStyle('QAA', parent=styles.get('BodyText', styles['Normal']),
...
try:
    ...
    # for auto-index like your friend's app
...
if mode == "Student Chat":
    ...
    if render_student_tab is None:
        ...
        # make sure extension expected folders exist
        with ext_cwd():
            ...
            # read/create segments and (re)index
            ...
            with st.spinner("Initializing Student Library (YouTube + FAISS)…"):
                ...
                if new_indexes == 0:
                    st.caption("Student Library is up-to-date.")
                else:
                    st.info(f"Indexed {new_indexes} new lecture(s).")
            ...
            # hand exact args to the tab, like your friend's app does
            try:
                render_student_tab(
                    "data",                 # DATA_DIR
                    META_SEGMENTS,
