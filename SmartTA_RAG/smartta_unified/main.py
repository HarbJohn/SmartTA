...
# parent = smartta_unified/, parent.parent = SmartTA_RAG/, parent.parent.parent = SmartTA_Final/
_PACKAGE_DIR = Path(__file__).resolve().parent
_SMARTTA_RAG_DIR = _PACKAGE_DIR.parent
_PROJECT_ROOT = _SMARTTA_RAG_DIR.parent

# Add SmartTA_RAG to path 
if str(_SMARTTA_RAG_DIR) not in sys.path:
    sys.path.insert(0, str(_SMARTTA_RAG_DIR))
# Add project root to path
if str(_PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(_PROJECT_ROOT))
# Add SmartTA_Extensions to path
_EXT_DIR = _PROJECT_ROOT / "SmartTA_Extensions"
if _EXT_DIR.exists() and str(_EXT_DIR) not in sys.path:
...
def _render_mode_selector() -> str:
    ...
    # Add Feedback tab for students (no login required)
    ...
    with center:
        picked = st.radio("Mode Selection", labels, horizontal=True, label_visibility="collapsed")
...
def run() -> None:
    ...
    # Tell Extensions to skip their own styles - we'll use unified styles
    st.session_state["skip_extension_styles"] = True

    st.set_page_config(page_title="🧠 SmartTA Unified Assistant", layout="wide")
    # Force browser cache refresh with timestamp
    import time
    cache_bust = int(time.time())
    st.markdown(f"<!-- v{cache_bust} -->{BASE_STYLES}", unsafe_allow_html=True)
    st.markdown(
    ...
    # Load RAG indexes once per session
